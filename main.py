...
class BearingRateTracker:
    """Tracks bearing rate and predicts target aspect"""
    
    def __init__(self):
        self.bearing_history = {}  # target_id -> deque of (timestamp, bearing)
        self.max_history = 30  # Store last 30 samples
        self.aspect_history = {}  # target_id -> aspect angle over time
        self.crossing_predictions = {}  # target_id -> crossing prediction
        
    def update_target_bearing(self, target_id, bearing, timestamp):
        """Update bearing history for a target"""
        if target_id not in self.bearing_history:
            self.bearing_history[target_id] = deque(maxlen=self.max_history)
        
        self.bearing_history[target_id].append((timestamp, bearing))
        
        # Calculate bearing rate if we have enough history
        if len(self.bearing_history[target_id]) >= 2:
            return self._calculate_bearing_rate(target_id)
        return 0.0
    
    def _calculate_bearing_rate(self, target_id):
        """Calculate bearing rate in degrees per minute"""
        history = self.bearing_history[target_id]
        
        if len(history) < 2:
            return 0.0
        
        # Get oldest and newest bearings
        old_time, old_bearing = history[0]
        new_time, new_bearing = history[-1]
        
        # Calculate time difference in minutes
        time_diff_minutes = (new_time - old_time) / 1000.0 / 60.0
        
        if time_diff_minutes == 0:
            return 0.0
        
        # Calculate bearing change (handling 0-360 wrap-around)
        bearing_diff = new_bearing - old_bearing
        if bearing_diff > 180:
            bearing_diff -= 360
        ...
        # Degrees per minute
        bearing_rate = bearing_diff / time_diff_minutes
        
        return bearing_rate
    
    def predict_crossing(self, target_id, own_course, relative_bearing, range_nm):
        """Predict if target will cross ahead or astern"""
        bearing_rate = self._calculate_bearing_rate(target_id)
        
        if abs(bearing_rate) < 0.1:  # No significant bearing change
            return "STEADY", None, None
        
        # Calculate crossing prediction
        # Positive bearing rate = target moving right (increasing bearing)
        # Negative bearing rate = target moving left (decreasing bearing)
        
        # Simplified crossing logic
        if bearing_rate > 2.0:  # Significant right movement
            if 0 < relative_bearing < 180:
                crossing = "CROSSING AHEAD TO PORT"
            else:
                crossing = "CROSSING ASTERN TO STARBOARD"
        elif bearing_rate < -2.0:  # Significant left movement
            if 0 < relative_bearing < 180:
                crossing = "CROSSING AHEAD TO STARBOARD"
            else:
                crossing = "CROSSING ASTERN TO PORT"
        else:
            crossing = "PARALLEL COURSE"
        
        # Calculate time to crossing (simplified)
        time_to_crossing = None
        if abs(bearing_rate) > 0.5:
            # Angular distance to own ship's bow/stern (simplified)
            if "AHEAD" in crossing:
                angular_distance = min(abs(relative_bearing), abs(relative_bearing - 360))
            else:
                angular_distance = abs(relative_bearing - 180)
            
            time_to_crossing = angular_distance / abs(bearing_rate)  # minutes
        
        # Determine aspect (bow, beam, stern)
        aspect = self._calculate_aspect(relative_bearing, bearing_rate)
        
        self.crossing_predictions[target_id] = {
            "crossing_type": crossing,
            "time_to_crossing_min": time_to_crossing,
            "bearing_rate_deg_min": bearing_rate,
            "aspect": aspect
        }
        
        return crossing, time_to_crossing, aspect
    
    def _calculate_aspect(self, relative_bearing, bearing_rate):
        """Calculate target aspect based on bearing rate and bearing"""
        # Aspect determination logic:
        # - Bearing steady, range decreasing: Bow aspect
        # - Bearing changing rapidly, range steady: Beam aspect
        # - Bearing steady, range increasing: Stern aspect
        
        bearing_rate_abs = abs(bearing_rate)
        
        if bearing_rate_abs < 1.0:
            return "BOW/STERN"
        elif 1.0 <= bearing_rate_abs < 5.0:
            return "QUARTER"
        else:
            return "BEAM"
    
    def get_bearing_drift_indicator(self, target_id):
        """Generate bearing drift visualization data"""
        if target_id not in self.bearing_history:
            return None
        
        history = list(self.bearing_history[target_id])
        if len(history) < 3:
            return None
        
        # Calculate trend line
        timestamps = [t for t, _ in history]
        bearings = [b for _, b in history]
        
        # Simple moving average
        window = min(5, len(bearings))
        if window >= 3:
            trend = []
            for i in range(len(bearings) - window + 1):
                trend.append(np.mean(bearings[i:i+window]))
            
            drift_direction = "RIGHT" if trend[-1] > trend[0] else "LEFT"
            drift_magnitude = abs(trend[-1] - trend[0])
            ...
            return {
                ...
                "current_bearing": bearings[-1]
            }
        
        return None
    
    def check_rapid_bearing_change(self, target_id, threshold=10.0):
        """Check for rapid bearing changes (possible collision course)"""
        bearing_rate = abs(self._calculate_bearing_rate(target_id))
        
        if bearing_rate > threshold:
            # Calculate collision risk
            history = self.bearing_history.get(target_id, deque())
            if len(history) >= 3:
                recent_bearings = [b for _, b in list(history)[-3:]]
                bearing_change = abs(recent_bearings[-1] - recent_bearings[0])
                
                if bearing_change > 5.0:  # More than 5 degrees in last 3 samples
                    return {
                        "alert": "RAPID BEARING CHANGE",
                        "bearing_rate": bearing_rate,
                        "risk": "HIGH" if bearing_rate > 15 else "MEDIUM",
                        "recommendation": "Monitor for collision risk"
                    }
        
        return None

# Initialize bearing tracker
bearing_tracker = BearingRateTracker()
...
class SNRAnalyzer:
    """Calculates and displays Signal-to-Noise Ratio for targets"""
    
    def __init__(self):
        self.target_snr_history = {}  # target_id -> deque of SNR values
        self.background_noise_level = -70.0  # dB (estimated background noise)
        self.snr_thresholds = {
            "VERY_POOR": 0,    # 0-6 dB
            "POOR": 6,         # 6-12 dB
            "FAIR": 12,        # 12-18 dB
    ...
    def calculate_snr(self, target_signal_db, background_noise_db=None):
        """Calculate SNR in dB"""
        if background_noise_db is None:
            background_noise_db = self.background_noise_level
        
        # SNR = Signal Level - Noise Level
        snr_db = target_signal_db - background_noise_db
        
        return max(0.0, snr_db)  # SNR can't be negative in this simplified model
    
    def update_target_snr(self, target_id, signal_level_db, timestamp=None):
        """Update SNR for a specific target"""
        if timestamp is None:
            timestamp = time.time()
        
        snr_db = self.calculate_snr(signal_level_db)
        
        if target_id not in self.target_snr_history:
            self.target_snr_history[target_id] = deque(maxlen=20)
        
        self.target_snr_history[target_id].append({
            "timestamp": timestamp,
            "snr_db": snr_db,
            "signal_db": signal_level_db,
            "noise_db": self.background_noise_level
        })
        
        return snr_db
    
    def get_snr_quality(self, snr_db):
        """Get SNR quality rating"""
        if snr_db >= self.snr_thresholds["EXCELLENT"]:
            return "EXCELLENT"
        elif snr_db >= self.snr_thresholds["GOOD"]:
            return "GOOD"
        elif snr_db >= self.snr_thresholds["FAIR"]:
            return "FAIR"
        elif snr_db >= self.snr_thresholds["POOR"]:
            return "POOR"
        else:
            return "VERY_POOR"
    
    def get_detection_confidence(self, snr_db):
        """Calculate detection confidence based on SNR"""
        # Simplified confidence model
        if snr_db >= 24:
            confidence = 0.95 + (snr_db - 24) * 0.0025
        elif snr_db >= 18:
            confidence = 0.85 + (snr_db - 18) * 0.0167
        elif snr_db >= 12:
            confidence = 0.70 + (snr_db - 12) * 0.025
        elif snr_db >= 6:
            confidence = 0.40 + (snr_db - 6) * 0.05
        else:
            confidence = snr_db * 0.0667
        
        return min(1.0, max(0.0, confidence))
    
    def get_classification_quality(self, snr_db):
        """Get classification quality based on SNR"""
        if snr_db >= 18:
            return "HIGH"
        elif snr_db >= 12:
            return "MEDIUM"
    ...
    def generate_snr_display_data(self, target_id):
        """Generate formatted SNR display data for a target"""
        if target_id not in self.target_snr_history:
            return None
        
        history = list(self.target_snr_history[target_id])
        if not history:
            return None
        ...
        quality = self.get_snr_quality(snr_db)
        confidence = self.get_detection_confidence(snr_db)
        class_quality = self.get_classification_quality(snr_db)
        
        # Calculate SNR trend
        trend = "STEADY"
        if len(history) >= 3:
            recent_snr = [h["snr_db"] for h in history[-3:]]
            if recent_snr[-1] > recent_snr[0] + 2:
                trend = "IMPROVING"
            elif recent_snr[-1] < recent_snr[0] - 2:
                trend = "DEGRADING"
        
        return {
            "snr_db": snr_db,
            "signal_db": latest["signal_db"],
            "noise_db": latest["noise_db"],
            "quality": quality,
            "detection_confidence": confidence,
            "classification_quality": class_quality,
            "trend": trend,
            "history": history[-5:] if len(history) >= 5 else history
        }
    
    def update_background_noise(self, audio_data):
        """Update background noise estimate from audio data"""
        try:
            if len(audio_data) > 0:
                # Calculate RMS of audio data
                rms = np.sqrt(np.mean(audio_data**2))
                noise_db = 20 * np.log10(rms + 1e-10) + 100  # Simplified conversion
                
                # Smooth the noise estimate
                self.background_noise_level = 0.9 * self.background_noise_level + 0.1 * noise_db
        except:
            pass
...
class TorpedoDetector:
    """Detects torpedoes and manages counter-measures"""
    
    def __init__(self):
        self.detected_torpedoes = {}  # torpedo_id -> torpedo_data
        self.torpedo_alerts = deque(maxlen=10)
        self.next_torpedo_id = 1000  # Start IDs at 1000 to distinguish from regular targets
        self.acoustic_decoys = []
        self.last_deploy_time = 0
        self.decoy_cooldown = 30  # seconds
        
        # Torpedo signature characteristics
        self.torpedo_signatures = {
            "HIGH_SPEED": {
                "min_speed": 35,  # knots
                "max_speed": 70,
                "frequency_range": (500, 3000),  # Hz
                "modulation": "steady",
                "signature_markers": ["High-speed propeller", "Cavitation noise", "Wake turbulence"]
            },
            "WAKEFUL": {
                "min_speed": 25,
                "max_speed": 45,
                ...
                "modulation": "pulsed",
                "signature_markers": ["Wake homing", "Intermittent pings"]
            }
        }
    
    def analyze_for_torpedoes(self, target_id, target_data, audio_signature=None):
        """Analyze target for torpedo characteristics"""
        torpedo_probability = 0.0
        torpedo_type = None
        
        # Check speed (primary indicator)
        speed_knots = abs(target_data.velocity * 3600 * (50 / RADAR_RADIUS))
        
        if speed_knots >= self.torpedo_signatures["HIGH_SPEED"]["min_speed"]:
            speed_match = min(1.0, (speed_knots - 35) / 35)
            
            # Check frequency if audio signature available
            freq_match = 0.5  # Default
            if audio_signature:
                dominant_freq = audio_signature.dominant_frequency
                if (self.torpedo_signatures["HIGH_SPEED"]["frequency_range"][0] <= 
                    dominant_freq <= 
                    self.torpedo_signatures["HIGH_SPEED"]["frequency_range"][1]):
                    freq_match = 1.0
            
            torpedo_probability = 0.7 * speed_match + 0.3 * freq_match
            torpedo_type = "HIGH_SPEED"
        
        # Check for wake homing characteristics
        elif speed_knots >= 25 and audio_signature:
            if audio_signature.modulation_pattern == "pulsed":
                torpedo_probability = 0.6
                torpedo_type = "WAKEFUL"
        
        return torpedo_probability, torpedo_type
    
    def detect_torpedo(self, target_id, target_data, torpedo_probability, torpedo_type):
        """Register a detected torpedo"""
        if torpedo_probability < 0.5:  # Threshold
            return None
        
        torpedo_id = self.next_torpedo_id
        self.next_torpedo_id += 1
        
        # Calculate impact parameters
        impact_data = self._calculate_impact_parameters(target_data)
        
        torpedo_data = {
            "id": torpedo_id,
            "original_target_id": target_id,
            "type": torpedo_type,
            "probability": torpedo_probability,
            "bearing": target_data.angle,
            "range_nm": target_data.get_range_nm(),
            "speed_knots": abs(target_data.velocity * 3600 * (50 / RADAR_RADIUS)),
        ...
        # Generate alert
        alert = self._generate_torpedo_alert(torpedo_data)
        self.torpedo_alerts.append(alert)
        
        # Play warning sound
        sound_system.play_sound("warning", volume=1.0)
        
        print(f"[TORPEDO] Detected! ID: {torpedo_id}, Type: {torpedo_type}, Prob: {torpedo_probability:.0%}")
        
        return torpedo_id
    
    def _calculate_impact_parameters(self, target_data):
        """Calculate torpedo impact parameters"""
        # Simplified impact calculation
        range_nm = target_data.get_range_nm()
        speed_knots = abs(target_data.velocity * 3600 * (50 / RADAR_RADIUS))
        
        # Assume torpedo is coming directly at us (worst case)
        if speed_knots > 0:
            time_to_impact_minutes = (range_nm / speed_knots) * 60
            time_to_impact_seconds = time_to_impact_minutes * 60
            
            # Format time as MM:SS
            minutes = int(time_to_impact_minutes)
            seconds = int((time_to_impact_minutes - minutes) * 60)
            
            impact_time_str = f"{minutes}:{seconds:02d}"
        else:
            time_to_impact_seconds = 9999
            impact_time_str = "N/A"
        
        # Calculate evasion recommendations
        ...
        return {
            "time_to_impact_sec": time_to_impact_seconds,
            "time_to_impact_str": impact_time_str,
            "bearing": target_data.angle,
            "range_nm": range_nm,
            "speed_knots": speed_knots,
            "recommendations": recommendations,
            "estimated_impact_point": self._estimate_impact_point(target_data)
        }
    
    def _generate_evasion_recommendations(self, bearing, speed_knots, time_to_impact):
        """Generate evasion recommendations"""
        recommendations = []
        
        # Turn recommendation based on bearing
        if 0 <= bearing < 180:
            recommendations.append("Turn HARD TO STARBOARD")
        else:
            recommendations.append("Turn HARD TO PORT")
        
        # Speed recommendation
        recommendations.append("FLANK SPEED")
        
        # Countermeasure recommendation
        if time_to_impact > 120:  # More than 2 minutes
            recommendations.append("Deploy acoustic decoys")
        else:
            recommendations.append("IMMEDIATE decoy deployment")
        
        # Additional tactical recommendations
        if speed_knots > 50:
            recommendations.append("Prepare for high-speed evasion")
        
        if time_to_impact < 60:
            recommendations.append("EMERGENCY: Brace for impact")
        
        return recommendations
    
    def _estimate_impact_point(self, target_data):
        """Estimate likely impact point"""
        # Simplified estimation - assume linear intercept
        pixels_to_nm = 50 / RADAR_RADIUS
        
        # Convert to Cartesian coordinates
        range_nm = target_data.get_range_nm()
        bearing_rad = math.radians(target_data.angle)
        
        target_x = range_nm * math.sin(bearing_rad)
        target_y = range_nm * math.cos(bearing_rad)
        
        # Estimate closure rate (simplified)
        closure_rate = 0.7 * abs(target_data.velocity * 3600 * pixels_to_nm)  # Assume 70% of speed is toward us
        
        if closure_rate > 0:
            intercept_time = range_nm / closure_rate * 3600  # seconds
        else:
            intercept_time = 9999
        ...
        return {
            "x_nm": target_x,
            "y_nm": target_y,
            "intercept_time_sec": intercept_time,
            "danger_radius_nm": 0.5  # Estimated danger radius
        }
    
    def _generate_torpedo_alert(self, torpedo_data):
        """Generate torpedo alert message"""
        impact = torpedo_data["impact_data"]
        
        alert = {
            "timestamp": time.time(),
            "torpedo_id": torpedo_data["id"],
            "type": torpedo_data["type"],
            "message": f"⚠️ TORPEDO IN WATER ⚠️",
            "details": [
                f"Bearing: {torpedo_data['bearing']:.0f}° RELATIVE",
                f"Speed: ~{torpedo_data['speed_knots']:.0f} knots",
                f"Time to Impact: {impact['time_to_impact_str']}",
                f"Range: {torpedo_data['range_nm']:.1f} NM"
            ],
    ...
    def deploy_acoustic_decoy(self):
        """Deploy acoustic decoy"""
        current_time = time.time()
        
        if current_time - self.last_deploy_time < self.decoy_cooldown:
            print(f"[DECOY] Cooldown active: {self.decoy_cooldown - (current_time - self.last_deploy_time):.0f}s remaining")
            return False
        
        decoy_id = len(self.acoustic_decoys) + 1
        
        decoy = {
            "id": decoy_id,
            "deploy_time": current_time,
            "position": {
                "bearing": own_ship["heading"] + random.uniform(-30, 30),
                "range_nm": 0.2  # 0.2 NM from ship
            },
        ...
        self.acoustic_decoys.append(decoy)
        self.last_deploy_time = current_time
        
        # Update torpedoes to consider decoy
        for torpedo_id, torpedo in self.detected_torpedoes.items():
            if torpedo["status"] == "TRACKING":
                # Chance to divert torpedo
                if random.random() < decoy["effectiveness"]:
                    torpedo["status"] = "DIVERTED"
                    torpedo["countermeasures_deployed"].append({
                        "decoy_id": decoy_id,
                        "time": current_time,
                        "successful": True
                    })
                    decoy["targets_diverted"].append(torpedo_id)
        
        print(f"[DECOY] Deployed decoy #{decoy_id}")
        sound_system.play_sound("ping", volume=0.5)
        
        return True
    
    def update_torpedo_tracking(self):
        """Update all tracked torpedoes"""
        current_time = time.time()
        to_remove = []
        
        for torpedo_id, torpedo in self.detected_torpedoes.items():
            # Update impact time
            elapsed = current_time - torpedo["detection_time"]
            impact = torpedo["impact_data"]
            
            if impact["time_to_impact_sec"] > elapsed:
                new_ttl = impact["time_to_impact_sec"] - elapsed
                minutes = int(new_ttl / 60)
                seconds = int(new_ttl % 60)
                torpedo["impact_data"]["time_to_impact_str"] = f"{minutes}:{seconds:02d}"
                
                # Check if impact imminent
                if new_ttl < 30 and torpedo["status"] == "TRACKING":
                    alert = {
                        "timestamp": current_time,
                        "message": f"TORPEDO {torpedo_id} IMPACT IMMINENT!",
                        "time_remaining": new_ttl,
                        "severity": "EMERGENCY"
                    }
                    self.torpedo_alerts.append(alert)
            else:
                # Torpedo has "impacted" or passed
                torpedo["status"] = "EXPIRED"
                to_remove.append(torpedo_id)
        
        # Remove expired torpedoes
        for torpedo_id in to_remove:
            del self.detected_torpedoes[torpedo_id]
    
    def get_active_torpedoes(self):
        """Get list of active torpedoes"""
        return [t for t in self.detected_torpedoes.values() if t["status"] == "TRACKING"]
    
    def get_recent_alerts(self, count=5):
        """Get recent torpedo alerts"""
        return list(self.torpedo_alerts)[-count:]

# Initialize torpedo detector
torpedo_detector = TorpedoDetector()
# ============================================================================
# SOUND SYSTEM
# ============================================================================
...
class AcousticSignature:
    """Represents the acoustic signature of a target"""
    dominant_frequency: float  # Hz
    frequency_range: Tuple[float, float]  # Min and max frequencies
    modulation_pattern: str  # "steady", "pulsed", "irregular"
    harmonic_content: List[float]  # Harmonic frequencies
    source_level_db: float  # Estimated source level
    pulse_interval: Optional[float] = None  # For pulsed sources
...
class TargetClassifier:
    """Classifies targets based on acoustic signatures"""
    ...
    CLASS_TEMPLATES = {
        ...
        "MERCHANT_VESSEL": {
            ...
            "category": "SURFACE",
            "description": "Large commercial ship with slow, low-frequency propeller",
            "acoustic_profile": {
                "dominant_frequency": (10, 80),  # Hz
                "frequency_range": (5, 200),
                "modulation_pattern": "steady",
                "harmonic_content": [60, 120, 180],
                "source_level_range": (160, 190),  # dB
                "speed_range": (5, 20),  # knots
                "signature_markers": ["Low rumble", "Regular propeller", "Machinery hum"]
            },
        ...
        "FAST_ATTACK_CRAFT": {
            ...
            "category": "SURFACE",
            "description": "Small, fast military vessel with high RPM",
            "acoustic_profile": {
                "dominant_frequency": (200, 600),
                "frequency_range": (100, 1000),
                "modulation_pattern": "irregular",
                "harmonic_content": [400, 800],
                "source_level_range": (140, 170),
                "speed_range": (20, 50),
                "signature_markers": ["High RPM", "Cavitation noise", "Water jet"]
            },
        ...
        "NAVAL_WARSHIP": {
            "name": "Naval Warship",
            "category": "SURFACE",
            "description": "Military surface combatant",
            "acoustic_profile": {
                "dominant_frequency": (50, 200),
                "frequency_range": (20, 500),
                "modulation_pattern": "complex",
                "harmonic_content": [100, 200, 300],
                "source_level_range": (150, 180),
                "speed_range": (10, 35),
                "signature_markers": ["Gas turbine", "Helicopter deck", "Sonar dome"]
            },
            "threat_level": "HOSTILE",
            "icon": "🚢"
        },
        "FISHING_VESSEL": {
            "name": "Fishing Vessel",
            "category": "SURFACE",
            "description": "Commercial fishing boat",
            "acoustic_profile": {
                "dominant_frequency": (30, 150),
                "frequency_range": (20, 300),
                "modulation_pattern": "irregular",
                "harmonic_content": [75, 150],
                "source_level_range": (130, 160),
                "speed_range": (3, 15),
                "signature_markers": ["Diesel engine", "Winch noise", "Irregular pattern"]
            },
            "threat_level": "NEUTRAL",
            "icon": "🎣"
        },
        ...
        "SUBMARINE_DIESEL": {
            ...
            "category": "SUBSURFACE",
            "description": "Conventional diesel-electric submarine",
            "acoustic_profile": {
                "dominant_frequency": (20, 100),
                "frequency_range": (10, 300),
                "modulation_pattern": "pulsed",
                "harmonic_content": [50, 100, 150],
                "source_level_range": (120, 160),
                "speed_range": (2, 20),
                "signature_markers": ["Diesel charging", "Electric motor", "Snorkel"],
                "pulse_interval": (1.0, 5.0)  # seconds between snorkel cycles
            },
            "threat_level": "HOSTILE",
            "icon": "🛳️"
        },
        "SUBMARINE_NUCLEAR": {
            ...
            "category": "SUBSURFACE",
            "description": "Nuclear-powered submarine",
            "acoustic_profile": {
                "dominant_frequency": (5, 50),
                "frequency_range": (2, 150),
                "modulation_pattern": "steady",
                "harmonic_content": [25, 50, 75],
                "source_level_range": (110, 150),
                "speed_range": (5, 35),
                "signature_markers": ["Reactor coolant pumps", "Steam turbines", "Very quiet"]
            },
            "threat_level": "HOSTILE",
            "icon": "🚤"
        },
        "TORPEDO": {
            "name": "Torpedo",
            "category": "SUBSURFACE",
            "description": "High-speed underwater weapon",
            "acoustic_profile": {
                "dominant_frequency": (500, 2000),
                "frequency_range": (200, 5000),
                "modulation_pattern": "steady",
                "harmonic_content": [1000, 2000],
                "source_level_range": (150, 180),
                "speed_range": (30, 70),
                "signature_markers": ["High-speed propeller", "Wake noise", "Rapid closure"]
            },
            "threat_level": "HOSTILE",
            "icon": "⚡"
        },
        "UUV": {
            ...
            "category": "SUBSURFACE",
            "description": "Remotely operated or autonomous vehicle",
            "acoustic_profile": {
                "dominant_frequency": (100, 500),
                "frequency_range": (50, 1000),
                "modulation_pattern": "irregular",
                "harmonic_content": [200, 400],
                "source_level_range": (100, 140),
                "speed_range": (1, 10),
                "signature_markers": ["Electric motor", "Thruster noise", "Intermittent"]
            },
        ...
        "WHALE": {
            ...
            "acoustic_profile": {
                "dominant_frequency": (10, 100),
                "frequency_range": (5, 500),
                "modulation_pattern": "irregular",
                "harmonic_content": [],
                "source_level_range": (140, 180),
                "speed_range": (1, 10),
                "signature_markers": ["Low moans", "Song patterns", "Breaching noise"]
            },
        ...
        "DOLPHIN_POD": {
            "name": "Dolphin Pod",
            "category": "BIOLOGICAL",
            "description": "Group of dolphins",
            "acoustic_profile": {
                "dominant_frequency": (2000, 15000),
                "frequency_range": (1000, 20000),
                "modulation_pattern": "irregular",
                "harmonic_content": [],
                "source_level_range": (120, 160),
                "speed_range": (5, 25),
                "signature_markers": ["Clicks", "Whistles", "Echolocation"]
            },
            "threat_level": "FRIENDLY",
            "icon": "🐬"
        },
        "FISH_SCHOOL": {
            "name": "Fish School",
            "category": "BIOLOGICAL",
            "description": "Large group of fish",
            "acoustic_profile": {
                "dominant_frequency": (100, 1000),
                "frequency_range": (50, 2000),
                "modulation_pattern": "irregular",
                "harmonic_content": [],
                "source_level_range": (90, 130),
                "speed_range": (0, 5),
                "signature_markers": ["Bubbles", "Swim bladder", "Movement"]
            },
            "threat_level": "NEUTRAL",
            "icon": "🐟"
        },
        ...
        "SEISMIC_SURVEY": {
            "name": "Seismic Survey",
            "category": "NON-BIOLOGICAL",
            "description": "Geological survey vessel",
            "acoustic_profile": {
                "dominant_frequency": (5, 50),
                "frequency_range": (2, 200),
                "modulation_pattern": "pulsed",
                "harmonic_content": [],
                "source_level_range": (180, 230),
                "speed_range": (3, 8),
                "signature_markers": ["Airgun pulses", "Regular intervals", "Low frequency"],
                "pulse_interval": (5.0, 15.0)
            },
            "threat_level": "NEUTRAL",
            "icon": "📡"
        },
        "CONTAINER_SHIP": {
            "name": "Container Ship",
            "category": "NON-BIOLOGICAL",
            "description": "Large cargo vessel on shipping lane",
            "acoustic_profile": {
                "dominant_frequency": (15, 60),
                "frequency_range": (10, 150),
                "modulation_pattern": "steady",
                "harmonic_content": [30, 60, 90],
                "source_level_range": (170, 200),
                "speed_range": (15, 25),
                "signature_markers": ["Slow turning", "Deep draft", "Bow wave"]
            },
            "threat_level": "NEUTRAL",
            "icon": "📦"
        },
        "WEATHER_NOISE": {
            "name": "Weather Noise",
            "category": "NON-BIOLOGICAL",
            "description": "Environmental noise from weather",
            "acoustic_profile": {
                "dominant_frequency": (100, 5000),
                "frequency_range": (10, 10000),
                "modulation_pattern": "irregular",
                "harmonic_content": [],
                "source_level_range": (80, 140),
                "speed_range": (0, 0),
                "signature_markers": ["Rain", "Waves", "Wind"]
            },
            "threat_level": "NEUTRAL",
            "icon": "🌊"
        }
    ...
    @staticmethod
    def classify(signature: AcousticSignature, velocity: float, bearing_rate: float) -> Dict:
        """Classify target based on acoustic signature and movement"""
        best_match = None
        best_score = 0.0
        possible_matches = []
        
        for class_id, template in TargetClassifier.CLASS_TEMPLATES.items():
            score = TargetClassifier._calculate_match_score(signature, velocity, bearing_rate, template)
            
            if score > 0.3:  # Threshold for possible match
        ...
        if best_match and best_score > 0.6:  # Confidence threshold
            return {
                "primary_classification": best_match,
                "confidence": best_score,
                "possible_classes": possible_matches[:3],  # Top 3 possibilities
            ...
            return {
                ...
                "confidence": 0.0,
                "possible_classes": possible_matches[:3],
                "classification_time": time.time()
            }
    
    @staticmethod
    def _calculate_match_score(signature: AcousticSignature, velocity: float, 
                              bearing_rate: float, template: Dict) -> float:
        """Calculate match score between signature and template"""
        profile = template["acoustic_profile"]
        score = 0.0
        
        # 1. Frequency match (40% weight)
        freq_min, freq_max = profile["dominant_frequency"]
        if freq_min <= signature.dominant_frequency <= freq_max:
            freq_score = 1.0
        else:
            # Calculate proximity score
            if signature.dominant_frequency < freq_min:
                freq_score = max(0, 1.0 - (freq_min - signature.dominant_frequency) / freq_min)
            else:
                freq_score = max(0, 1.0 - (signature.dominant_frequency - freq_max) / freq_max)
        score += freq_score * 0.4
        
        # 2. Speed match (20% weight)
        speed_min, speed_max = profile["speed_range"]
        if speed_min <= velocity <= speed_max:
            speed_score = 1.0
        else:
            speed_score = max(0, 1.0 - abs(velocity - (speed_min + speed_max) / 2) / 50)
        score += speed_score * 0.2
        
        # 3. Modulation pattern match (15% weight)
        if signature.modulation_pattern == profile["modulation_pattern"]:
            mod_score = 1.0
        elif profile["modulation_pattern"] == "irregular":
            mod_score = 0.7  # Irregular can match many patterns
        else:
            mod_score = 0.3
        score += mod_score * 0.15
        
        # 4. Source level match (15% weight)
        level_min, level_max = profile["source_level_range"]
        if level_min <= signature.source_level_db <= level_max:
            level_score = 1.0
        else:
            level_score = max(0, 1.0 - abs(signature.source_level_db - (level_min + level_max) / 2) / 50)
        score += level_score * 0.15
        
        # 5. Pulse interval match (10% weight, if applicable)
        if "pulse_interval" in profile and signature.pulse_interval:
            pulse_min, pulse_max = profile["pulse_interval"]
            if pulse_min <= signature.pulse_interval <= pulse_max:
                pulse_score = 1.0
            else:
                pulse_score = max(0, 1.0 - abs(signature.pulse_interval - (pulse_min + pulse_max) / 2) / 10)
            score += pulse_score * 0.1
        else:
            score += 0.1  # Bonus for non-pulsed
        
        # Apply bearing rate filter for biological vs mechanical
        if template["category"] == "BIOLOGICAL" and abs(bearing_rate) > 5:
            score *= 0.5  # Biological typically don't turn fast
        
        return min(1.0, score)
    
    @staticmethod
    def generate_signature_from_audio(audio_data: np.ndarray, sample_rate: int = 44100) -> AcousticSignature:
        """Generate acoustic signature from audio data"""
        ...
        if len(significant_indices) > 0:
            ...
            min_freq = dominant_freq - 10
            max_freq = dominant_freq + 10
        ...
        for i in range(2, 5):
            ...
            if harmonic_idx < len(magnitudes):
                harmonic_strength = magnitudes[harmonic_idx] / magnitudes[dominant_idx]
                if harmonic_strength > 0.1:
                    harmonics.append(harmonic_freq)
        
        # Detect modulation pattern
        envelope = np.abs(signal.hilbert(audio_mono))
        envelope_variation = np.std(envelope) / np.mean(envelope)
        
        if envelope_variation < 0.1:
            modulation = "steady"
        elif envelope_variation < 0.3:
            modulation = "pulsed"
        else:
            modulation = "irregular"
        
        # Calculate pulse interval if pulsed
        pulse_interval = None
        if modulation == "pulsed":
            peaks, _ = signal.find_peaks(envelope, height=np.mean(envelope)*0.5)
            if len(peaks) > 1:
                pulse_interval = np.mean(np.diff(peaks)) / sample_rate
        
        # Estimate source level (simplified)
        rms = np.sqrt(np.mean(audio_mono**2))
        source_level_db = 20 * np.log10(rms + 1e-10) + 120  # Rough approximation
        ...
        return AcousticSignature(
            ...
            doppler_shift=0.0  # Would need consecutive samples to calculate
        )
...
# Target detection cooldown tracking
last_detection_time = {}
...
class Target:
    ...
    def __init__(self, angle, distance, intensity, sound_types, threat_level, detection_mode):
        ...
        # NEW TACTICAL FEATURES - ADD THESE LINES:
        self.closing_rate = 0.0
        self.closing_rate_type = "STEADY"
        self.cpa_distance = 0.0
        self.time_to_cpa = 999.0
        ...
        self.acoustic_signature = None
        self.classification_confidence = 0.0
        self.classification_time = None
    ...
    def update_classification(self, audio_data=None, sample_rate=44100):
        """Update target classification based on audio data"""
        if audio_data is not None:
            # Generate acoustic signature
            self.acoustic_signature = target_classifier.generate_signature_from_audio(
                audio_data, sample_rate
            )
            
            # Calculate velocity for classification (convert to knots)
            pixels_to_nm = 50 / RADAR_RADIUS
            velocity_nm_per_sec = self.velocity * pixels_to_nm
            velocity_knots = velocity_nm_per_sec * 3600
            
            # Classify
            classification_result = target_classifier.classify(
                self.acoustic_signature,
                velocity_knots,
                self.angular_velocity
            )
            
            self.classification = classification_result["primary_classification"]
            self.classification_confidence = classification_result["confidence"]
            self.possible_classifications = classification_result["possible_classes"]
            self.classification_time = classification_result["classification_time"]
            ...
            # Update threat level based on classification if confidence is high
            if self.classification_confidence > 0.7:
                self.threat_level = self.classification["threat_level"]
    ...
    def calculate_tactical_data(self):
        """Calculate closing rate and CPA"""
        try:
            # Get own ship data
            own_speed = own_ship["speed"]
            own_course = own_ship["course"]
            
            # Convert pixel velocity to knots
            pixels_to_nm = 50 / RADAR_RADIUS
            velocity_nm_per_sec = self.velocity * pixels_to_nm
            self.estimated_speed = abs(velocity_nm_per_sec * 3600)
            
            # Calculate closing rate
            target_speed_knots = velocity_nm_per_sec * 3600
            own_course_rad = math.radians(own_course)
            own_vx = own_speed * math.sin(own_course_rad)
            own_vy = own_speed * math.cos(own_course_rad)
            
            target_course_rad = self.velocity_angle
            target_vx = target_speed_knots * math.sin(target_course_rad)
            target_vy = target_speed_knots * math.cos(target_course_rad)
            
            rel_vx = target_vx - own_vx
            rel_vy = target_vy - own_vy
            
            bearing_rad = math.radians(self.angle)
            bearing_vx = math.sin(bearing_rad)
            bearing_vy = math.cos(bearing_rad)
            
            self.closing_rate = -(rel_vx * bearing_vx + rel_vy * bearing_vy)
            
            # Classify closing rate
            if self.closing_rate > 15:
                self.closing_rate_type = "RAPID CLOSE"
            elif self.closing_rate > 5:
                self.closing_rate_type = "CLOSING"
            elif self.closing_rate > -5:
                self.closing_rate_type = "STEADY"
            elif self.closing_rate > -15:
                self.closing_rate_type = "OPENING"
            else:
                self.closing_rate_type = "RAPID OPEN"
            
            # Calculate CPA
            current_range_nm = self.distance * pixels_to_nm
            target_bearing_rad = math.radians(self.angle)
            target_x = current_range_nm * math.sin(target_bearing_rad)
            target_y = current_range_nm * math.cos(target_bearing_rad)
            
            own_vx = own_speed * math.sin(own_course_rad) / 3600
            own_vy = own_speed * math.cos(own_course_rad) / 3600
            
            target_vx = target_speed_knots * math.sin(target_course_rad) / 3600
            target_vy = target_speed_knots * math.cos(target_course_rad) / 3600
            ...
            if rel_speed > 0.0001:
                time_to_cpa_sec = -(target_x * rel_vx + target_y * rel_vy) / (rel_vx**2 + rel_vy**2)
                
                if time_to_cpa_sec > 0:
                    self.time_to_cpa = time_to_cpa_sec / 60
                    cpa_x = target_x + rel_vx * time_to_cpa_sec
                    cpa_y = target_y + rel_vy * time_to_cpa_sec
                    self.cpa_distance = math.sqrt(cpa_x**2 + cpa_y**2)
                    
                    # *** NEW: Calculate CPA bearing for visualization ***
                    self.cpa_bearing = math.degrees(math.atan2(cpa_x, cpa_y)) % 360
                else:
                    self.time_to_cpa = 0
                    self.cpa_distance = current_range_nm
                    self.cpa_bearing = self.angle  # *** NEW ***
            else:
                self.time_to_cpa = 999.0
                self.cpa_distance = current_range_nm
                self.cpa_bearing = self.angle  # *** NEW ***
            ...
            print(f"[ERROR] Tactical calculation error: {e}")
            self.closing_rate = 0.0
            self.closing_rate_type = "STEADY"
            self.cpa_distance = 0.0
            self.time_to_cpa = 999.0
...
class TargetManager:
    ...
    def update_or_create(self, angle, distance, intensity, sound_types, threat_level, detection_mode, audio_data=None):
        ...
        if audio_data is not None:
            new_target.update_classification(audio_data)
...
def analyze_audio():
    ...
    try:
        ...
        intensity = min(intensity * 50.0, 1.0)
        ...
        # Convert angle to degrees for detection logic
        detected_angle = math.degrees(angle)
        ...
        # Only proceed if we have significant intensity
        if intensity > detection_threshold and detected_sounds:
            config = ModeConfig.get_config(current_mode)
            should_detect = False
            ...
            # Create target if detection conditions are met
            if should_detect:
                dist = min(intensity * 15 * config["range_multiplier"], RADAR_RADIUS * 0.9)
                types = [s["type"] for s in detected_sounds]
                ...
                target = target_manager.update_or_create(
                    ...
                    threat_level,
                    current_mode,
                    audio_data  # Pass audio data for classification
                )
                ...
                if target:
                    ...
                    # Play detection sound with cooldown
                    if (target.id not in last_detection_time or 
                        current_time - last_detection_time.get(target.id, 0) > 1000):  # 1 second cooldown
...
# ============================================================================
def draw_classification_panel(panel, target):
    """Draw target classification panel"""
    if panel.collapsed or not target:
        panel.draw(screen)
        return
    ...
    # Target header
    header_text = font_header.render(f"TARGET T{target.id}", True, get_color("accent"))
    screen.blit(header_text, (panel.rect.x + 10, y))
    y += 20
    
    # Primary classification
    if target.classification:
        # Icon and name
        class_icon = font_header.render(target.classification.get("icon", "?"), True, get_color("primary"))
        screen.blit(class_icon, (panel.rect.x + 10, y))
        
        class_text = font_data.render(target.classification["name"], True, get_color("primary"))
        screen.blit(class_text, (panel.rect.x + 25, y))
        
        y += 18
        
        # Category
        category_text = font_data.render(f"Category: {target.classification['category']}", 
                                        True, get_color("accent"))
        screen.blit(category_text, (panel.rect.x + 10, y))
        y += 16
        
        # Confidence
        conf_color = get_color("accent") if target.classification_confidence > 0.7 else get_color("warning")
        conf_text = font_data.render(f"Confidence: {target.classification_confidence:.0%}", 
                                    True, conf_color)
        screen.blit(conf_text, (panel.rect.x + 10, y))
        y += 16
        ...
        threat_color = {
            ...
            "NEUTRAL": get_color("primary"),
            "FRIENDLY": get_color("accent")
        }.get(target.threat_level, get_color("primary"))
        
        threat_text = font_data.render(f"Threat: {target.threat_level}", 
                                      True, threat_color)
        screen.blit(threat_text, (panel.rect.x + 10, y))
        y += 20
        
        # Possible classifications
        if target.possible_classifications:
            poss_text = font_header.render("POSSIBLE CLASSES:", True, get_color("primary"))
            screen.blit(poss_text, (panel.rect.x + 10, y))
            y += 15
            
            for i, poss in enumerate(target.possible_classifications[:3]):
                if poss["class_id"] != target.classification["class_id"]:
                    poss_line = f"  • {poss['name']} ({poss['score']:.0%})"
                    poss_surf = font_data.render(poss_line, True, get_color("primary"))
                    screen.blit(poss_surf, (panel.rect.x + 10, y))
                    y += 14
        
        # Acoustic signature details
        if target.acoustic_signature:
            y += 10
            sig_text = font_header.render("ACOUSTIC SIGNATURE:", True, get_color("primary"))
            screen.blit(sig_text, (panel.rect.x + 10, y))
            y += 15
            
            sig_data = [
                f"Freq: {target.acoustic_signature.dominant_frequency:.0f} Hz",
                f"Modulation: {target.acoustic_signature.modulation_pattern}",
                f"Source: {target.acoustic_signature.source_level_db:.0f} dB"
            ]
            
            for line in sig_data:
                line_surf = font_data.render(line, True, get_color("accent"))
                screen.blit(line_surf, (panel.rect.x + 10, y))
                y += 14
    
    else:
        # No classification yet
        no_class_text = font_data.render("No classification data", True, get_color("warning"))
        screen.blit(no_class_text, (panel.rect.x + 10, y))
        y += 20
        
        # Classification button
        class_btn = pygame.Rect(panel.rect.x + 10, y, 120, 20)
        pygame.draw.rect(screen, get_color("secondary"), class_btn)
        pygame.draw.rect(screen, get_color("primary"), class_btn, 1)
        
        btn_text = font_data.render("[C] Classify", True, get_color("accent"))
        screen.blit(btn_text, (panel.rect.x + 15, y + 4))
        
        y += 30
        
        help_text = font_data.render("Select target and press C", True, get_color("primary"))
        screen.blit(help_text, (panel.rect.x + 10, y))
...
panels = [
    ...
    DraggablePanel(320, 210, 300, 300, "TARGET CLASSIFICATION"),
    DraggablePanel(10, 280, 300, 220, "BEARING RATE TRACKER"),
    DraggablePanel(320, 520, 300, 200, "SNR ANALYSIS"),
    DraggablePanel(10, 510, 300, 180, "TORPEDO DEFENSE")
]
...
def draw_target(target):
    ...
    # Draw classification icon if available
    if target.classification and target.classification_confidence > 0.5:
        font_icon = pygame.font.SysFont("Segoe UI Emoji", 12)
        icon = target.classification.get("icon", "?")
        icon_surf = font_icon.render(icon, True, color)
        screen.blit(icon_surf, (int(pos[0]) - 6, int(pos[1]) - 20))
...
def draw_tracked_targets(panel):
    ...
    for target in sorted_targets[:20]:
        color = target.get_color()
         ...
         # Calculate tactical data first
        target.calculate_tactical_data()
        
        # Color by closing rate
        if hasattr(target, 'closing_rate'):
            if target.closing_rate > 5:
                color = get_color("danger")  # Red for closing fast
            elif target.closing_rate < -5:
                color = get_color("primary")  # Green for opening
            else:
                color = get_color("warning")  # Yellow for steady
        else:
            color = target.get_color()
        ...
        data = [
            ...
            f"{int(target.angle)}°",
            f"{target.closing_rate:+.0f}" if hasattr(target, 'closing_rate') else "0",
            f"{target.cpa_distance:.1f}" if hasattr(target, 'cpa_distance') else "0"
        ]
...
def handle_keyboard_shortcuts(event):
    ...
    # Noise gate
        ...
        noise_gate = min(0.9, noise_gate + 0.01)  # Smaller increments
        print(f"[NOISE GATE] {noise_gate:.3f}")
    elif event.key == pygame.K_m:
        noise_gate = max(0.0, noise_gate - 0.01)  # Smaller increments
        print(f"[NOISE GATE] {noise_gate:.3f}")
    ...
    # Export analytics
        ...
        # Deploy acoustic decoy
        if torpedo_detector.deploy_acoustic_decoy():
            print("[TORPEDO] Deployed acoustic decoy")
        sound_system.play_ui_click()

    elif event.key == pygame.K_w:
        # Toggle bearing rate display
        global show_bearing_rate
        show_bearing_rate = not show_bearing_rate
        print(f"[BEARING] Rate display {'enabled' if show_bearing_rate else 'disabled'}")
        sound_system.play_ui_click()
...
def draw_cpa_visualization():
    ...
    # Draw 2 NM danger circle around own ship
    pixels_to_nm = 50 / RADAR_RADIUS
    danger_radius_pixels = 2.0 / pixels_to_nm  # 2 NM danger zone
    
    # Draw danger circle (red, dashed)
    pygame.draw.circle(screen, get_color("danger"), RADAR_CENTER, 
                      int(danger_radius_pixels * zoom_level), 2)
    
    # Draw CPA lines for each target
    for target in target_manager.targets.values():
        # Skip if no CPA data calculated yet
        if not hasattr(target, 'cpa_distance') or target.cpa_distance <= 0:
            continue
        ...
        # CPA bearing and distance are already calculated in target.calculate_tactical_data()
        cpa_bearing = target.cpa_bearing if hasattr(target, 'cpa_bearing') else target.angle
        cpa_distance_pixels = target.cpa_distance / pixels_to_nm
        
        # Calculate CPA point on screen
        cpa_x = RADAR_CENTER[0] + cpa_distance_pixels * math.cos(math.radians(cpa_bearing - 90)) * zoom_level
        cpa_y = RADAR_CENTER[1] + cpa_distance_pixels * math.sin(math.radians(cpa_bearing - 90)) * zoom_level
        
        # Determine color based on danger
        if target.cpa_distance < 2.0:
            # DANGEROUS - Red
            line_color = get_color("danger")
            cpa_marker_color = (255, 0, 0)
            draw_line = True
        elif target.cpa_distance < 5.0:
            # WARNING - Yellow
            line_color = get_color("warning")
            cpa_marker_color = (255, 255, 0)
            draw_line = True
        elif target.closing_rate > 5:
            # CLOSING - Orange
            line_color = (255, 165, 0)
            cpa_marker_color = (255, 165, 0)
            draw_line = True
        else:
            # SAFE - Don't draw CPA line for safe targets
            draw_line = False
        
        if draw_line and target.time_to_cpa < 60:  # Only draw if CPA within 60 minutes
            # Draw line from target to CPA point
            try:
                pygame.draw.line(screen, line_color, 
                               (int(target_x), int(target_y)), 
                               (int(cpa_x), int(cpa_y)), 2)
                
                # Draw CPA marker (circle)
                pygame.draw.circle(screen, cpa_marker_color, 
                                 (int(cpa_x), int(cpa_y)), 8, 2)
                
                # Draw X inside circle
                pygame.draw.line(screen, cpa_marker_color,
                               (int(cpa_x-5), int(cpa_y-5)),
                               (int(cpa_x+5), int(cpa_y+5)), 2)
                pygame.draw.line(screen, cpa_marker_color,
                               (int(cpa_x-5), int(cpa_y+5)),
                               (int(cpa_x+5), int(cpa_y-5)), 2)
                
                # Draw label showing CPA distance
                font = pygame.font.SysFont("Courier New", 9, bold=True)
                label_text = f"CPA T{target.id}: {target.cpa_distance:.1f}NM"
                label_surf = font.render(label_text, True, cpa_marker_color)
                screen.blit(label_surf, (int(cpa_x) + 12, int(cpa_y) - 5))
                
                # If dangerous, show time to CPA
                if target.cpa_distance < 2.0 and target.time_to_cpa < 999:
                    time_text = f"{target.time_to_cpa:.0f}min"
                    time_surf = font.render(time_text, True, cpa_marker_color)
                    screen.blit(time_surf, (int(cpa_x) + 12, int(cpa_y) + 5))
                    
            except Exception as e:
                print(f"[ERROR] CPA visualization error for target {target.id}: {e}")
...
def draw_bearing_rate_panel(panel):
    """Draw bearing rate tracker panel"""
    if panel.collapsed:
        panel.draw(screen)
        return
    ...
    # Panel title
    title = font_header.render("BEARING RATE TRACKER", True, get_color("accent"))
    screen.blit(title, (panel.rect.x + 10, y))
    y += 20
    
    # Tracked targets
    for target_id, target in list(target_manager.targets.items())[:6]:  # Show first 6
        # Update bearing tracker
        bearing_rate = bearing_tracker.update_target_bearing(
            target_id, 
            target.angle, 
            pygame.time.get_ticks()
        )
        
        # Get crossing prediction
        crossing, time_to_cross, aspect = bearing_tracker.predict_crossing(
            target_id,
            own_ship["course"],
            target.angle,
            target.get_range_nm()
        ...
        # Check for rapid bearing changes
        rapid_change = bearing_tracker.check_rapid_bearing_change(target_id)
        
        # Display target info
        target_text = font_data.render(f"T{target_id}: {bearing_rate:+.1f}°/min", 
                                      True, get_color("primary"))
        screen.blit(target_text, (panel.rect.x + 10, y))
        
        # Aspect indicator
        if aspect:
            aspect_color = get_color("warning") if aspect == "BEAM" else get_color("primary")
            aspect_text = font_data.render(aspect, True, aspect_color)
            screen.blit(aspect_text, (panel.rect.x + 120, y))
        
        # Crossing prediction
        if "CROSSING" in crossing:
            cross_color = get_color("danger") if "AHEAD" in crossing else get_color("warning")
            cross_text = font_data.render(crossing[:15], True, cross_color)
            screen.blit(cross_text, (panel.rect.x + 10, y + 12))
            
            if time_to_cross:
                time_text = font_data.render(f"{time_to_cross:.0f}min", True, cross_color)
                screen.blit(time_text, (panel.rect.x + 200, y + 12))
        
        # Rapid change alert
        if rapid_change:
            alert_text = font_data.render("⚠️", True, get_color("danger"))
            screen.blit(alert_text, (panel.rect.x + panel.rect.width - 20, y))
        
        y += 30
    
    # Drift visualization for selected target
    if target_manager.selected_target:
        drift_data = bearing_tracker.get_bearing_drift_indicator(
            target_manager.selected_target.id
        )
        
        if drift_data:
            y += 10
            drift_text = font_header.render(f"T{target_manager.selected_target.id} DRIFT:", 
                                          True, get_color("accent"))
            screen.blit(drift_text, (panel.rect.x + 10, y))
            y += 15
            
            direction = drift_data["direction"]
            magnitude = drift_data["magnitude"]
            
            drift_info = f"{direction} {magnitude:.1f}°"
            info_text = font_data.render(drift_info, True, get_color("primary"))
            screen.blit(info_text, (panel.rect.x + 20, y))
            
            # Simple drift indicator bar
            bar_width = min(100, int(magnitude * 10))
            bar_color = get_color("danger") if magnitude > 5 else get_color("warning")
            pygame.draw.rect(screen, bar_color, 
                           (panel.rect.x + 130, y, bar_width, 8))

def draw_snr_panel(panel):
    """Draw SNR analysis panel"""
    if panel.collapsed:
        panel.draw(screen)
        return
    ...
    # Panel title
    title = font_header.render("SNR ANALYSIS", True, get_color("accent"))
    screen.blit(title, (panel.rect.x + 10, y))
    y += 20
    
    # Background noise level
    noise_text = font_data.render(f"Background Noise: {snr_analyzer.background_noise_level:.1f} dB", 
                                 True, get_color("primary"))
    screen.blit(noise_text, (panel.rect.x + 10, y))
    y += 20
    
    # Selected target SNR
    if target_manager.selected_target:
        target_id = target_manager.selected_target.id
        
        # Calculate signal level (simplified from intensity)
        signal_db = 20 * math.log10(target_manager.selected_target.intensity + 1e-10) + 100
        
        # Update SNR
        snr_db = snr_analyzer.update_target_snr(target_id, signal_db)
        snr_data = snr_analyzer.generate_snr_display_data(target_id)
        
        if snr_data:
            # SNR value with quality color
            quality_color = {
                "EXCELLENT": get_color("accent"),
                "GOOD": get_color("primary"),
                "FAIR": get_color("warning"),
                "POOR": get_color("danger"),
                "VERY_POOR": (100, 100, 100)
            }.get(snr_data["quality"], get_color("primary"))
            
            snr_text = font_header.render(f"Target T{target_id}: SNR = {snr_db:.0f} dB", 
                                         True, quality_color)
            screen.blit(snr_text, (panel.rect.x + 10, y))
            y += 15
            
            # Detailed breakdown
            details = [
                f"├─ Signal: {snr_data['signal_db']:.0f} dB",
                f"├─ Noise: {snr_data['noise_db']:.0f} dB",
                f"├─ Detection Confidence: {snr_data['detection_confidence']:.0%}",
                f"└─ Classification Quality: {snr_data['classification_quality']}"
            ]
            
            for detail in details:
                detail_text = font_data.render(detail, True, get_color("primary"))
                screen.blit(detail_text, (panel.rect.x + 15, y))
                y += 14
            
            y += 5
            
            # SNR trend
            trend_color = get_color("accent") if snr_data["trend"] == "IMPROVING" else get_color("warning")
            trend_text = font_data.render(f"Trend: {snr_data['trend']}", True, trend_color)
            screen.blit(trend_text, (panel.rect.x + 15, y))
            y += 16
            
            # SNR history bar graph (simplified)
            if snr_data["history"]:
                y += 5
                hist_title = font_data.render("SNR History:", True, get_color("primary"))
                screen.blit(hist_title, (panel.rect.x + 15, y))
                y += 12
                
                # Draw mini bar graph
                history_values = [h["snr_db"] for h in snr_data["history"]]
                max_snr = max(history_values) if history_values else 1
                
                for i, snr_val in enumerate(history_values[-10:]):  # Last 10 values
                    bar_height = int((snr_val / max_snr) * 30) if max_snr > 0 else 0
                    bar_x = panel.rect.x + 20 + (i * 12)
                    bar_y = y + 30 - bar_height
                    
                    # Color based on SNR value
                    if snr_val >= 18:
                        bar_color = get_color("accent")
                    elif snr_val >= 12:
                        bar_color = get_color("primary")
                    elif snr_val >= 6:
                        bar_color = get_color("warning")
                    else:
                        bar_color = get_color("danger")
                    
                    pygame.draw.rect(screen, bar_color, 
                                   (bar_x, bar_y, 8, bar_height))
        
        else:
            no_data = font_data.render("No SNR data for selected target", 
                                      True, get_color("warning"))
            screen.blit(no_data, (panel.rect.x + 10, y))
    else:
        select_text = font_data.render("Select a target for SNR analysis", 
                                      True, get_color("primary"))
        screen.blit(select_text, (panel.rect.x + 10, y))

def draw_torpedo_panel(panel):
    """Draw torpedo detection panel"""
    if panel.collapsed:
        panel.draw(screen)
        return
    ...
    # Panel title
    title = font_header.render("TORPEDO DEFENSE", True, get_color("danger"))
    screen.blit(title, (panel.rect.x + 10, y))
    y += 20
    
    # Update torpedo tracking
    torpedo_detector.update_torpedo_tracking()
    
    # Active torpedoes
    active_torpedoes = torpedo_detector.get_active_torpedoes()
    
    if active_torpedoes:
        active_text = font_header.render(f"ACTIVE TORPEDOES: {len(active_torpedoes)}", 
                                        True, get_color("danger"))
        screen.blit(active_text, (panel.rect.x + 10, y))
        y += 20
        
        for torpedo in active_torpedoes[:3]:  # Show first 3
            # Torpedo info
            info_text = font_data.render(f"Torp {torpedo['id']}: {torpedo['type']}", 
                                        True, get_color("danger"))
            screen.blit(info_text, (panel.rect.x + 15, y))
            
            # Bearing and range
            bearing_text = font_data.render(f"{torpedo['bearing']:.0f}° / {torpedo['range_nm']:.1f}NM", 
                                           True, get_color("warning"))
            screen.blit(bearing_text, (panel.rect.x + 120, y))
            y += 12
            
            # Time to impact
            impact_text = font_data.render(f"Impact: {torpedo['impact_data']['time_to_impact_str']}", 
                                          True, get_color("danger"))
            screen.blit(impact_text, (panel.rect.x + 15, y))
            y += 20
            
            # Draw separator
            pygame.draw.line(screen, get_color("danger"), 
                           (panel.rect.x + 10, y), 
                           (panel.rect.x + panel.rect.width - 10, y), 1)
            y += 10
        ...
        y += 5
        decoy_text = font_header.render("COUNTERMEASURES:", True, get_color("accent"))
        screen.blit(decoy_text, (panel.rect.x + 10, y))
        y += 15
        
        # Decoy cooldown
        current_time = time.time()
        cooldown_remaining = torpedo_detector.decoy_cooldown - (current_time - torpedo_detector.last_deploy_time)
        
        if cooldown_remaining > 0:
            cooldown_text = font_data.render(f"Decoy cooldown: {cooldown_remaining:.0f}s", 
                                            True, get_color("warning"))
            screen.blit(cooldown_text, (panel.rect.x + 15, y))
        else:
            ready_text = font_data.render("Decoys READY [Q to deploy]", 
                                         True, get_color("accent"))
            screen.blit(ready_text, (panel.rect.x + 15, y))
        
        y += 16
        
        # Active decoys
        if torpedo_detector.acoustic_decoys:
            active_decoys = [d for d in torpedo_detector.acoustic_decoys 
                           if time.time() - d["deploy_time"] < d["active_duration"]]
            
            if active_decoys:
                decoy_count = font_data.render(f"Active decoys: {len(active_decoys)}", 
                                              True, get_color("primary"))
                screen.blit(decoy_count, (panel.rect.x + 15, y))
    
    else:
        # No active torpedoes
        clear_text = font_header.render("NO TORPEDOES DETECTED", True, get_color("primary"))
        screen.blit(clear_text, (panel.rect.x + 10, y))
        y += 25
        
        # Monitoring status
        monitor_text = font_data.render("Monitoring for torpedo signatures...", 
                                       True, get_color("primary"))
        screen.blit(monitor_text, (panel.rect.x + 15, y))
        y += 20
        
        # Auto-analysis info
        auto_text = font_data.render("Auto-analyzing high-speed targets", 
                                    True, get_color("accent"))
        screen.blit(auto_text, (panel.rect.x + 15, y))
    
    # Recent alerts
    recent_alerts = torpedo_detector.get_recent_alerts(3)
    if recent_alerts:
        y += 30
        alert_title = font_header.render("RECENT ALERTS:", True, get_color("warning"))
        screen.blit(alert_title, (panel.rect.x + 10, y))
        y += 15
        
        for alert in recent_alerts:
            time_ago = time.time() - alert["timestamp"]
            if time_ago < 60:  # Only show alerts from last minute
                alert_text = font_data.render(alert["message"][:30], True, get_color("warning"))
                screen.blit(alert_text, (panel.rect.x + 15, y))
                y += 14

def draw_torpedo_alerts():
    """Draw on-screen torpedo alerts"""
    active_torpedoes = torpedo_detector.get_active_torpedoes()
    ...
    font_medium = pygame.font.SysFont("Courier New", 16, bold=True)
    
    # Draw flashing alert box
    current_time = pygame.time.get_ticks()
    flash_alpha = 128 + 127 * math.sin(current_time / 200)  # Flashing effect
    
    alert_surface = pygame.Surface((WIDTH, 80), pygame.SRCALPHA)
    alert_color = (*get_color("danger")[:3], int(flash_alpha))
    pygame.draw.rect(alert_surface, alert_color, (0, 0, WIDTH, 80))
    pygame.draw.rect(alert_surface, get_color("accent"), (0, 0, WIDTH, 80), 3)
    
    screen.blit(alert_surface, (0, HEIGHT // 2 - 40))
    
    # Alert text
    alert_text = font_large.render("⚠️ TORPEDO ALERT ⚠️", True, (255, 255, 255))
    text_rect = alert_text.get_rect(center=(WIDTH // 2, HEIGHT // 2 - 10))
    screen.blit(alert_text, text_rect)
    
    # Count text
    count_text = font_medium.render(f"{len(active_torpedoes)} ACTIVE", True, (255, 255, 255))
    count_rect = count_text.get_rect(center=(WIDTH // 2, HEIGHT // 2 + 20))
    screen.blit(count_text, count_rect)

def draw_target(target):
    """Draw a single target"""
    # TORPEDO CHECK - Check if target is detected as torpedo
    is_torpedo = False
    torpedo_data = None
    for torpedo in torpedo_detector.detected_torpedoes.values():
        if torpedo.get("original_target_id") == target.id:
            is_torpedo = True
    ...
    # TORPEDO RENDERING - Special rendering for torpedoes
    if is_torpedo and torpedo_data:
        # Draw torpedo as red diamond with warning symbol
        size = 12
        points = [
            (pos[0], pos[1] - size),
            (pos[0] + size, pos[1]),
        ...
        pygame.draw.polygon(screen, get_color("danger"), points)
        
        # Draw warning symbol
        warning_font = pygame.font.SysFont("Courier New", 14, bold=True)
        warning_text = warning_font.render("!", True, (255, 255, 255))
        warning_rect = warning_text.get_rect(center=(pos[0], pos[1]))
        screen.blit(warning_text, warning_rect)
        
        # Draw speed trail
        if abs(target.velocity) > 1:
            trail_length = min(abs(target.velocity) * 5, 100)
            trail_end_x = pos[0] + trail_length * math.cos(target.velocity_angle)
            trail_end_y = pos[1] + trail_length * math.sin(target.velocity_angle)
            pygame.draw.line(screen, get_color("danger"), pos, (trail_end_x, trail_end_y), 3)
        
        # Draw impact countdown
        if torpedo_data.get("status") == "TRACKING":
            impact_time = torpedo_data.get("impact_data", {}).get("time_to_impact_str", "N/A")
            countdown_font = pygame.font.SysFont("Courier New", 10, bold=True)
            countdown_text = countdown_font.render(impact_time, True, get_color("danger"))
            screen.blit(countdown_text, (pos[0] + 15, pos[1] - 25))
        
        # Skip regular target rendering for torpedoes
        return
    
    # Draw classification icon if available (regular targets only)
    if target.classification and target.classification_confidence > 0.5:
        font_icon = pygame.font.SysFont("Segoe UI Emoji", 12)
        icon = target.classification.get("icon", "?")
        icon_surf = font_icon.render(icon, True, color)
    ...
    # Draw target symbol (regular targets only)
    size = 8
    if target_manager.selected_target == target:
        size = 12
        # Draw selection ring
    ...
    # Draw target icon based on threat (regular targets only)
    if target.threat_level == "HOSTILE":
        # Triangle
        points = [
            (pos[0], pos[1] - size),
    ...
    # Draw ID label (regular targets only)
    font = pygame.font.SysFont("Courier New", 9, bold=True)
    id_text = font.render(f"T{target.id}", True, color)
    screen.blit(id_text, (int(pos[0] + 10), int(pos[1] - 10)))
    
    # Draw velocity vector - SIMPLIFIED with numpy type conversion (regular targets only)
    if abs(target.velocity) > 1:
        vec_length = min(abs(target.velocity) * 2, 50)
    ...
    # Draw predicted positions (regular targets only)
    for pred_angle, pred_dist in target.predicted_positions:
        if math.isnan(pred_angle) or math.isnan(pred_dist):
            continue
...
def main():
    ...
    while running:
        ...
        # Update background noise level for SNR analyzer
        if audio_enabled and len(audio_queue) > 0:
            try:
                audio_data = audio_queue[-1]
                if audio_data.shape[1] == 2:
                    audio_mono = np.mean(audio_data, axis=1)
                    ...
                    audio_mono = audio_data[:, 0]
                snr_analyzer.update_background_noise(audio_mono)
            except:
                pass

        # Torpedo detection for all targets
        for target_id, target in target_manager.targets.items():
            # Calculate speed in knots
            speed_knots = abs(target.velocity * 3600 * (50 / RADAR_RADIUS))
            
            # Only check high-speed targets
            if speed_knots > 25:  # 25+ knots is suspicious
                torpedo_prob, torpedo_type = torpedo_detector.analyze_for_torpedoes(
                    target_id, 
                    target, 
                ...
                if torpedo_prob > 0.5:
                    torpedo_detector.detect_torpedo(target_id, target, torpedo_prob, torpedo_type)
        ...
        detection_threshold = 0.001   # Very low threshold
        ...
        for i, panel in enumerate(panels):
            ...
            elif i == 8:  # Bearing Rate panel
                draw_bearing_rate_panel(panel)
            elif i == 9:  # SNR Analysis panel
                draw_snr_panel(panel)
            elif i == 10: # Torpedo Defense panel
                draw_torpedo_panel(panel) 
