...
class SoundSystem:
    ...
    def play_radar_sweep(self):
        """Play radar sweep sound based on mode"""
        if not self.sound_initialized:
            return
    ...
    def play_target_detection(self, target_threat):
        """Play sound for target detection based on threat level"""
        if not self.sound_initialized:
            return
            
        if target_threat == "HOSTILE":
            self.play_sound("warning", volume=0.7)
            # Also play ping for immediate feedback
            self.play_sound("ping", volume=0.4)
        elif target_threat == "UNKNOWN":
            self.play_sound("ping", volume=0.5)
        elif target_threat == "FRIENDLY":
            self.play_sound("ping", volume=0.3)
        else:  # NEUTRAL
            self.play_sound("ping", volume=0.2)
...
def main():
    ...
    # Variables for sweep timing
    last_sweep_sound_time = 0
    sweep_sound_interval = 1000  # ms between sweep sounds
    ...
    while running:
        ...
        # Play periodic sweep sound
        current_time = pygame.time.get_ticks()
        config = ModeConfig.get_config(current_mode)
        
        # Adjust sweep sound interval based on mode
        if config["sweep_speed"] > 0:
            sweep_sound_interval = max(200, 1000 // config["sweep_speed"])
        
        if current_time - last_sweep_sound_time > sweep_sound_interval:
            sound_system.play_radar_sweep()
            last_sweep_sound_time = current_time
        ...
        if current_noise_data["intensity"] > detection_threshold and current_noise_data["detected_sounds"]:
            ...
            # In the main loop, around where target detection happens, add:
            if should_detect:
